exports.run = async (client, msg, args) => {
 ...
 description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  ...
  if(member.id === "459621456612032513")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`**:no_entry_sign: Hoop! Yapımcıma Sövemezsin!!!**`)
}}) 
   if(member.id === "472319217224646667")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`**:no_entry_sign: Bu Kisi Kurucum Tarafindan Korunuyor!**!`)
}}) 
  if(member.id === "498087782628196382")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`**:no_entry_sign: Bu Kisi Kurucum Tarafindan Korunuyor**!`)
}}) 
