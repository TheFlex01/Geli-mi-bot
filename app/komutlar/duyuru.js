...
exports.run = (client, message, args, member) => {
  ...
  if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("❌Duyuru Yapmak İçin Üst Rolde Olman Gerekli Kanka");
    if (mesaj.length < 1) return message.channel.send(':x: Duyuruya yazmak için bir şeyler yazınız.');
  let incidentchannel = message.guild.channels.find(`name`, "『🔔』∂υуυяυℓαя");
